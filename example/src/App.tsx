...
import { today, toHijri, toGregorian, getDaysInMonth, fromTimestamp } from 'hijri-native';

const hijriToday = today('Asia/Riyadh');
const hijriFromGreg = toHijri(2026, 2, 18);
const backToGreg = toGregorian(hijriFromGreg.year, hijriFromGreg.month, hijriFromGreg.day);
...
export default function App() {
  return (
    <View style={styles.container}>
      ...
      <Text>{hijriToday.day}/{hijriToday.month}/{hijriToday.year}</Text>

      <Text style={styles.label}>toHijri(2026, 2, 18):</Text>
      <Text>{hijriFromGreg.day}/{hijriFromGreg.month}/{hijriFromGreg.year}</Text>

      <Text style={styles.label}>back to Gregorian:</Text>
      <Text>{backToGreg.day}/{backToGreg.month}/{backToGreg.year}</Text>

      <Text style={styles.label}>days in current month:</Text>
      <Text>{daysInMonth}</Text>

      <Text style={styles.label}>fromTimestamp (Mecca):</Text>
      <Text>{fromTs.day}/{fromTs.month}/{fromTs.year}</Text>
    </View>
  );
}
